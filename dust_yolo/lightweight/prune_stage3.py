...
def shrink_c3str_embed(module, new_embed, device="cpu"):
    ...
    old_cv3 = module.cv3
    cv3_out = old_cv3.conv.out_channels
...
def shrink_mlp_ratio(module, new_ratio, device="cpu"):
    embed = module.cv1.conv.out_channels
    old_mlp_dim = module.m.tr[0].mlp.fc1.out_features
    new_mlp_dim = embed * new_ratio

    if new_mlp_dim >= old_mlp_dim:
        return

    for tr in module.m.tr:
        f1 = nn.Linear(embed, new_mlp_dim).to(device)
        f2 = nn.Linear(new_mlp_dim, embed).to(device)
        with torch.no_grad():
            f1.weight.copy_(tr.mlp.fc1.weight[:new_mlp_dim, :embed])
            f1.bias.copy_(tr.mlp.fc1.bias[:new_mlp_dim])
            f2.weight.copy_(tr.mlp.fc2.weight[:embed, :new_mlp_dim])
            f2.bias.copy_(tr.mlp.fc2.bias[:embed])
        tr.mlp.fc1 = f1
        tr.mlp.fc2 = f2


def reduce_c3_depth(module, new_n):
    old_n = len(module.m)
    if new_n >= old_n:
        return old_n, old_n
    module.m = module.m[:new_n]
    return old_n, len(module.m)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--weights", type=str, required=True)
    parser.add_argument("--save", type=str, default="pruned_stage3.pt")
    ...
    shrink_c3str_embed(model.model[24], 64, device)

    for i in [21, 24, 27, 30]:
        shrink_mlp_ratio(model.model[i], 2, device)

    reduce_c3_depth(model.model[6], 6)
